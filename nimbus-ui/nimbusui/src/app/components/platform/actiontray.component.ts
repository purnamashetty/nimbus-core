...
@Component({
    ...
    animations: [
        trigger('actionTrayAnimation', [
            state('trayFixed', style({
                position: 'fixed',
                top: 0
            })),
            state('trayScroll', style({
                position: 'absolute',
                top: '100px'
            }))
        ]),
...
export class ActionTray extends BaseElement implements OnDestroy {
    ...
    onWindowScroll() {
        ...
        } else if (window.pageYOffset || document.documentElement.scrollTop || document.body.scrollTop < 100) { this.state = 'trayScroll'; }
    }
    ...
    isTrayVisible(){
        if(this.actionButtons){
            this.showTray = this.actionButtons.toArray().some(
                item =>
